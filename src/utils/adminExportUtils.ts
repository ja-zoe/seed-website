...
export function formatAdminDataForExport(proposals: ProjectProposal[]) {
  const exportData = {
    ...
    "Summary": proposals.map((proposal, index) => ({
      ...
      "Project Goal": proposal.goal.overarchingAim.substring(0, 100) + (proposal.goal.overarchingAim.length > 100 ? '...' : ''),
      "Total Budget": `$${proposal.expected_expenses.reduce((total, expense) => {
 ...
 */
export async function exportProposalsToExcel(proposals: ProjectProposal[], filename?: string) {
  try {
    ...
    const metadataSheet = XLSX.utils.aoa_to_sheet([
      ...
      ["Total Budget Requested:", `$${proposals.reduce((total, proposal) => {
        return total + proposal.expected_expenses.reduce((expenseTotal, expense) => {
          const cost = parseFloat(expense.cost.replace(/[$,]/g, '')) || 0;
          return expenseTotal + cost;
        }, 0);
      }, 0).toLocaleString()}`],
      ["Unique Project Leads:", new Set(proposals.flatMap(p => p.leads.map(l => l.email))).size.toString()],
    ]);
    XLSX.utils.book_append_sheet(workbook, metadataSheet, "Export Info");
    
    // Generate filename
    const defaultFilename = `seed-proposals-admin-export-${new Date().toISOString().split('T')[0]}`;
 ...
 */
export async function exportSingleProposal(proposal: ProjectProposal, filename?: string) {
